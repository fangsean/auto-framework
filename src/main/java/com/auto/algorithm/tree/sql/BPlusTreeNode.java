...
public class BPlusTreeNode implements Serializable {
    ...
    public int num;	// 关键字个数,最大为max个
    public int[] key;	// num个关键值
    public int[] addr;	// 关键值对应的数据指针()
    /* 结点指针 */
    public BPlusTreeNode[] children;	// fork下级结点,子结点。若子结点已在内存时，方便引用
    public BPlusTreeNode next;			// 同层的下一结点,便于遍历
